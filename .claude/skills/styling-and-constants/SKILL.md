...
Components don't hold magic values. Every shared or meaningful value has one home; the
...
| Section / landmark ids and `#hrefs`                                                                                                 | `src/constants/sections.ts` (`SECTION_IDS`, `sectionHref`) |
...
- A `const LABELS = { cs, en }` inside a component — move it to `src/data/uiText.ts`.
